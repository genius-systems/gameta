...
class GametaRepo(object):
    """
    Base interface for a repository initialised by the VCS interface. There are 2 possible ways to interact with the
    underlying VCS system either using the VCS interface wrapper directly using the self.interface property, or by a
    repository class generated by the VCS interface wrapper using the self.repo property.

    Attributes:
        interface (VCS): VCS interface wrapper
        ignore_file (File): File that holds all the data to be ignored
        repo (Optional[Any]): A repository class if provided
        details (Optional[Dict]): Repository details if provided
    """
    def __init__(
            self,
            interface: 'VCS',
