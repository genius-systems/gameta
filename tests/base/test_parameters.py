...
class TestParameter(TestCase):
    def test_parameter_substitute_parameters_missing(self):
        with self.assertRaisesRegex(CommandError, r"Parameter .* was not provided"):
    ...
    def test_parameter_substitute_shell_script(self):
        self.assertEqual(
            Parameter("cp {__shell__:echo 'hello'} {$DEST}").substitute(
                {"$DEST": "world"}
            ),
            "cp hello world",
        )
    ...
    def test_parameter_substitute_or_parameter(self):
        self.assertEqual(
            Parameter("cp {test|$TEST|TEST} {DEST}").substitute(
                {"TEST": "hello", "DEST": "world"}
            ),
            "cp hello world",
        )
    ...
    def test_parameter_substitute_or_shell_parameter(self):
        self.assertEqual(
            Parameter("cp {test|__shell__:echo 'hello'|TEST} {DEST}").substitute(
                {"TEST": "test", "DEST": "world"}
            ),
            "cp hello world",
        )
    ...
    def test_parameter_escaped_characters(self):
        self.assertEqual(
            Parameter(
                'export {{"asdf": "hello"}} >> test.json && cp test.json {DEST}'
            ).substitute({"DEST": "world"}),
