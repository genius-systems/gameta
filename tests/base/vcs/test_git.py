...
class TestGit(TestCase):
    ...
    def test_git_is_vcs_type_folder_is_git_repo(self):
        with self.runner.isolated_filesystem() as f:
            copytree(
                join(dirname(dirname(dirname(dirname(__file__)))), ".git"),
                join(f, ".git"),
            )
    ...
    def test_git_clone_invalid_url_provided(self):
        with self.runner.isolated_filesystem() as f:
            with self.assertRaisesRegex(
                VCSError, "Could not initialise git repository"
            ):
    ...
    def test_git_clone_repository_cloned(self):
        with self.runner.isolated_filesystem() as f:
            ...
            self.assertEqual(test_repo.name, "gameta")
            self.assertEqual(
                test_repo.url, "http://github.com/genius-systems/gameta.git"
            )
            self.assertEqual(test_repo.branch, "master")
...
class TestGitRepo(TestCase):
    ...
    def test_git_repo_create_with_gitignore(self):
        with self.runner.isolated_filesystem() as f:
            ...
            copyfile(
                join(dirname(dirname(dirname(__file__))), "data", ".gitignore"),
                join(f, ".gitignore"),
            )
            test_repo = Git.generate_repo(f)
    ...
    def test_git_repo_fetch_all_updates(self):
        with self.runner.isolated_filesystem() as f:
            with zipfile.ZipFile(
                join(dirname(dirname(dirname(__file__))), "data", "gitpython.zip"), "r"
            ) as template:
    ...
    def test_git_repo_switch_to_non_existent_branch(self):
        with self.runner.isolated_filesystem() as f:
            ...
            self.assertTrue(
                test_repo.repo.head.reference.name == test_repo.branch == "new_branch"
            )
    ...
    def test_git_repo_switch_to_existing_branch(self):
        with self.runner.isolated_filesystem() as f:
            ...
            self.assertTrue(
                test_repo.repo.head.reference.name == test_repo.branch == "master"
            )
            self.assertEqual(test_repo.repo.commit().hexsha, test_repo.hash)
    ...
    def test_git_repo_switch_to_existing_git_reference(self):
        with self.runner.isolated_filesystem() as f:
            ...
            self.assertTrue(
                test_repo.repo.head.reference.name == test_repo.branch == "HEAD~2"
            )
            self.assertTrue(
                test_repo.repo.commit().hexsha == test_repo.hash != old_hash
            )
    ...
    def test_git_repo_update_code_branch_does_not_exist(self):
        with self.runner.isolated_filesystem() as f:
            ...
            test_repo = Git.generate_repo(f)
            with self.assertRaisesRegex(
                VCSError, "invalid_branch does not exist in remote source origin"
            ):
    ...
    def test_git_repo_update_code_from_unrelated_histories(self):
        with self.runner.isolated_filesystem() as f:
            ...
            test_repo = Git.generate_repo(f)
            with self.assertRaisesRegex(
                VCSError, "refusing to merge unrelated histories"
            ):
