...
from gameta.base import Schema, supported_versions, to_schema_str, to_schema_tuple
...
class TestSupportedVersions(TestCase):
    def test_supported_versions(self):
        self.assertEqual(
            list(supported_versions), [(0, 2, 5), (0, 2, 6), (0, 2, 7), (0, 3, 0)]
        )
        self.assertTrue(all(isinstance(i, Schema) for i in supported_versions.values()))
        self.assertTrue(
            supported_versions[(0, 2, 5)]
            == supported_versions[(0, 2, 6)]
            == supported_versions[(0, 2, 7)]
        )
