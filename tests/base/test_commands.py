...
from gameta.base.commands import SHELL, CommandConfig, GametaCommand, Runner
...
class TestGametaCommand(TestCase):
    ...
    def test_gameta_command_generate_full_pipeline_with_virtualenv(self):
        test_inputs = {
            ...
            "config": {"shell": False, "venv": "path/to/venv"},
        }
        self.command = GametaCommand(
        ...
        with self.command.generate() as c:
            self.assertEqual(
                ...
                [
                    f"{SHELL}",
                    "-c",
                    ". path/to/venv/bin/activate && cp hello.yml test/hello.yml",
                ],
            )
    ...
    def test_gameta_command_substitute_single_parameter(self):
        ...
        self.assertEqual(
            self.command.substitute(), ["docker-compose -f hello.yml up -d"]
        )
        self.assertEqual(self.command.commands, test_inputs["commands"])
    ...
    def test_gameta_command_substitute_multiple_parameters(self):
        ...
        self.assertEqual(self.command.substitute(), ["cp hello.yml test/hello.yml"])
        self.assertEqual(self.command.commands, test_inputs["commands"])
    ...
    def test_gameta_command_shell_with_semicolon_separator(self):
        test_inputs = {
            ...
            "params": {},
            "config": {"shell": False, "venv": "path/to/venv", "sep": ";"},
        }
        self.command = GametaCommand(
            commands=test_inputs["commands"],
            params=test_inputs["params"],
...
class TestRunner(TestCase):
    ...
    def test_runner_apply_command_in_separate_shell(self):
        with self.runner.isolated_filesystem() as f:
            ...
            for cwd, repo, repo_command in zip(
                ...
                ["gameta", "genisys", "genisys-testing"],
                gameta_runner.apply(
                    ["git fetch --all --tags --prune", "git pull"], shell=True
                ),
            ):
                self.assertEqual(getcwd(), cwd)
                self.assertEqual(repo, repo_command[0])
            ...
            for cwd, test_output, repo, repo_command in zip(
                ...
                [
                    ...
                    [
                        "git",
                        "clone",
                        "https://github.com/test/genisys.git",
                        "core/genisys",
                    ],
                    [
                        "git",
                        "clone",
                        "https://github.com/test/genisys-testing.git",
                        "core/genisys-testing",
                    ],
                ],
                ["gameta", "genisys", "genisys-testing"],
            ...
            for cwd, test_output, repo, repo_command in zip(
                ...
                [
                    [
                        "git",
                        "clone",
                        "https://github.com/test/gameta.git",
                        ".",
                        "&&",
                        "git",
                        "checkout",
    ...
    def test_runner_apply_shell_command_with_virtualenv(self):
        with self.runner.isolated_filesystem() as f:
            ...
            for cwd, repo, repo_command in zip(
                ...
                ["gameta", "genisys", "genisys-testing"],
                gameta_runner.apply(
                    ["git fetch --all --tags --prune", "git pull"], venv="test"
                ),
            ):
                self.assertEqual(getcwd(), cwd)
                self.assertEqual(repo, repo_command[0])
    ...
    def test_runner_generate_parameters_complete_parameter_set_generated(self):
        ...
        gameta_runner.env_vars = {"$HELLO": "world", "$I": "am", "$A": "test"}
    ...
    def test_runner_generate_parameters_constants_substituted_by_environment_variables(
        self,
    ):
        gameta_runner = Runner(
            "",
    ...
    def test_runner_generate_parameters_parameters_substituted_by_environment_variables(
        self,
    ):
        gameta_runner = Runner(
            "",
