    ...
    def test_schema_cli_print_schema(self):
        ...
        with self.runner.isolated_filesystem():
            ...
            self.assertEqual(
                ...
                f"{pformat(supported_versions[to_schema_tuple(params['version'])].schema)}\n",
            )
...
class TestSchemaValidate(TestCase):
    ...
    def test_validate_gameta_schema_invalid(self):
        params = {
            ...
            "virtualenvs": {"hello!": "path/to/venv", "world!": "path/to/venv"},
    ...
    def test_validate_gameta_schema_valid(self):
        params = {
            ...
            "virtualenvs": {"hello_world": "path/to/venv"},
            "constants": {"HELLO": "world"},
        }
        with self.runner.isolated_filesystem() as f:
            copytree(join(dirname(__file__), "data", "gameta_multi_repos_only"), f)
...
class TestSchemaLs(TestCase):
    ...
    def test_schema_ls_all(self):
        with self.runner.isolated_filesystem():
            ...
            self.assertEqual(result.exit_code, 0)
            self.assertEqual(
                result.output, "Supported schema versions: 0.2.5, 0.2.6, 0.2.7, 0.3.0\n"
            )
